...
## Phase 5 (not started) — real Windows Update as the primary signal, multi-package-manager plugins
...
Reprioritization from that same live install: winget was never meant to be this checker's primary signal, actual Windows Update (OS-level updates, with real MSRC severity ratings, unlike winget's total lack of one) is. That detection doesn't exist yet -- `internal/checker/windows` today is winget-plus-registry-reboot-check only. Also not started: Scoop/Chocolatey as alternative Windows package sources, a Homebrew-based macOS checker, and Docker image update (tag/digest drift) detection on Linux. None of this phase has begun; captured here so it isn't lost, not as a commitment to any particular design yet.
