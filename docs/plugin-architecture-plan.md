...
**Phase 2 — Windows checker, no companion changes.**
`internal/checker/windows/*`, `golang.org/x/sys/windows/registry` dependency, wire into the (already-stubbed) `platforms_windows.go`/`hostflavor_windows.go`. Tests: winget-table-parsing logic is fixture-testable cross-platform (parsing func untagged, per design above); registry-key reboot detection genuinely cannot be tested without a real/virtual Windows box — **there is currently no Windows CI runner** (Forgejo's only runner is a single amd64 Linux nuc, per `release.yml`'s own comments) — call this an accepted testing gap for this phase, and land with an explicit "Windows checker: experimental" note in the README given winget's format-stability risk.
