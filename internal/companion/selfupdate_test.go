  ...
  inspect)
    case "$3" in
      '{{.Config.Image}}')
        ...
        for id in "$@"; do
          case "$id" in
            cid123) echo "ghcr.io/winarto/update-detector:v0.9.0" ;;
          esac
        done
        ;;
...
`)
	...
	if !strings.Contains(calls, "pull ghcr.io/winarto/update-detector:v0.11.0") {
		t.Fatalf("expected a pull of the target version, got log: %q", calls)
	}
	if !strings.Contains(calls, "tag ghcr.io/winarto/update-detector:v0.11.0 ghcr.io/winarto/update-detector:v0.9.0") {
