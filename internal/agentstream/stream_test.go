...
func TestRunInvokesOnAction(t *testing.T) {
	...
	go Run(ctx, srv.URL, aggregatorclient.Identity{AgentID: "agent-1", Token: "tok"}, aggregator.KindCompanion, func(a aggregator.Action) {
		received <- a
	})
...
func TestRunStopsOnContextCancellation(t *testing.T) {
	...
	go func() {
		Run(ctx, srv.URL, aggregatorclient.Identity{AgentID: "a", Token: "t"}, aggregator.KindAgent, func(aggregator.Action) {})
		close(done)
	}()
...
func TestRunOnceSendsRequestedKind(t *testing.T) {
	...
	if err := runOnce(ctx, srv.URL, aggregatorclient.Identity{AgentID: "a", Token: "t"}, aggregator.KindAgent, func(aggregator.Action) {}); err != nil {
...
func TestRunOnce409YieldsErrSuperseded(t *testing.T) {
	...
	err := runOnce(ctx, srv.URL, aggregatorclient.Identity{AgentID: "a", Token: "t"}, aggregator.KindAgent, func(aggregator.Action) {})
...
func TestRunOnceEventLineResetsOnBlankLine(t *testing.T) {
	...
	err := runOnce(ctx, srv.URL, aggregatorclient.Identity{AgentID: "a", Token: "t"}, aggregator.KindAgent, func(a aggregator.Action) {
		got = a
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
