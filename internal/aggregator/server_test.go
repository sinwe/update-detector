...
func TestHandleAdminNotifyDownMuteFor(t *testing.T) {
	s, reg := newTestServer(t)
	if _, _, err := reg.Enroll("a1", "web01", "tok"); err != nil {
		t.Fatal(err)
	}
	...
	rec := doJSON(t, s, http.MethodPost, "/admin/agents/a1/notify-down",
		notifyDownRequest{Enabled: false, MuteFor: "8h"}, nil)
	if rec.Code != http.StatusOK {
		t.Fatalf("got status %d, body %s, want 200", rec.Code, rec.Body.String())
	}
	...
	if resp["notify_down"] != false {
		t.Fatalf("expected effective notify_down=false while muted, got %s", rec.Body.String())
	}
	untilStr, _ := resp["muted_until"].(string)
	if untilStr == "" {
		t.Fatalf("expected muted_until in response, got %s", rec.Body.String())
	}
	got, _ := reg.Get("a1")
	if !got.NotifyDown || got.MutedUntil == nil {
	...
	if strings.Contains(rec.Body.String(), "muted_until") {
		t.Fatalf("expected mute cleared on re-enable, got %s", rec.Body.String())
	}

	// Contradictory or unparsable durations are rejected, as is unknown id.
	for _, bad := range []notifyDownRequest{
		{Enabled: true, MuteFor: "1h"},
		{Enabled: false, MuteFor: "bogus"},
		{Enabled: false, MuteFor: "0h"},
		{Enabled: false, MuteFor: "-1h"},
		{Enabled: false, MuteFor: "721h"},
	} {
		if rec := doJSON(t, s, http.MethodPost, "/admin/agents/a1/notify-down", bad, nil); rec.Code != http.StatusBadRequest {
			t.Fatalf("got status %d for %#v, want 400", rec.Code, bad)
		}
	}
